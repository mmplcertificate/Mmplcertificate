...
async function draftFromTemplate({ category, requestType, templateText, nitText, notes, certMeta, signingPartner, certificateLocation }) {
  ...
  const parts = [
    ...
    'Fixed fact: include an Annexure (starting "Annexure - A", ending with the same "For Singhi & Co." signature block repeated) only when this certificate\'s own subject matter is a computed figure the recipient needs to verify (e.g. Net Worth, Turnover, Working Capital, Solvency, Total Income, or a Local Content/shareholding percentage) - use a numbered table (e.g. SL No. / Particulars / Value in Rs. Lakhs) showing exactly how that figure was built up, matching the reference template\'s own table structure (same columns, same number of rows/years) whenever a template is provided below rather than simplifying it. A certificate that is a pure confirmation or negative-assurance statement about a fact rather than a computed figure (e.g. a "No CDR"/"No Corporate Debt Restructuring" certificate, or an "Audit Under Process" declaration) has NO Annexure at all - do not invent one for these.',
    `Fixed fact: this certificate's subject matter is defined by its category ("${category}") and, when provided, by what the reference template below actually certifies - certify only that subject, in the reference template's own terms. Do not substitute a different, unrelated certification just because other figures are available in this prompt: for example, a Local Content Certificate certifies the percentage/proportion of local content in the Company's goods or services, a "No CDR" certificate certifies the absence of Corporate Debt Restructuring, and a Shareholding Certificate certifies the shareholding pattern - none of these should mention Net Worth, Turnover, or Working Capital figures unless the reference template or tender document itself asks for them alongside its actual subject.`,
